...
class Lucene_Object(object):
      ...
      def get_coll_bigram_freq(self,bigram,field,ordered,slop,title,field_cache='id'):      
          if self.is_bigram_cache_used==True:
             ...
             if item_cf is not None:
                cf=item_cf['value']
                item_tf=self.conn_bigram_tf_cache.find_one({field_cache:title,'bigram':bigram,'field':field,'ordered':ordered,'slop':slop})
                ...
                pass
                #print ('not found')
                #print ({field_cache:title,'bigram':bigram,'field':field,'ordered':ordered,'slop':slop})
                #print ('not found cf')                 
          ...
          for leave in list_leaves:
              ...
              while spans.nextDoc()!=DocIdSetIterator.NO_MORE_DOCS:
                    ...
                    if self.is_bigram_cache_used==True:
                       item_tf=self.conn_bigram_tf_cache.find_one({'bigram':bigram,'field':field,'ordered':ordered,field_cache:id,'slop':slop})
                       if item_tf is not None:
                          continue 
                    ...
                    if self.is_bigram_cache_used==True:
                       self.conn_bigram_tf_cache.insert({field_cache:id,'bigram':bigram,'field':field,'ordered':ordered,'slop':slop,'value':doc_phrase_freq[id]})
